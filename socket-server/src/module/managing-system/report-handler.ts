...
const reportTrafficDistribution = (time: number) => ({ time, value: Math.round(MANAGING_SYSTEM_STATE.goalQualityIndicators[2].quality * 100) })
const reportBackUpPowerUsage = (time: number) => ({ time, value: Math.round(MANAGING_SYSTEM_STATE.goalQualityIndicators[1].quality * 100) })
