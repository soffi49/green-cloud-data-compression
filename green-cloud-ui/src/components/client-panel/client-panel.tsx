...
const JOB_STATUS_MAP = Object.keys(JobStatus).map((key) => {
   return { jobStatus: key.replaceAll('_', ' '), isSelected: true }
})
...
const ClientPanel = () => {
   ...
   const selectData = useMemo(
      () =>
         (clients as ClientAgent[])
            .filter(
               (client) =>
                  jobStatusMap.find(
                     (el) =>
                        el.jobStatus ===
                        client.jobStatusEnum.toString().replaceAll('_', ' ')
                  )?.isSelected
            )
            .reduce((prev, curr) => {
               const prevGroup = prev.find(
                  (el) => el.label === curr.jobStatusEnum.toString()
               )
               if (prevGroup) {
                  prevGroup.options.push({
            ...
            .sort(
               (a, b) =>
                  CLIENTS_ORDER.indexOf(a.label) -
                  CLIENTS_ORDER.indexOf(b.label)
            ),
