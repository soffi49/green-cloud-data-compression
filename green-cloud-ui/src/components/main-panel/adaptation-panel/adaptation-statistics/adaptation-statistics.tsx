...
import { ADAPTATION_STATISTICS_FIELDS, COUNTERS } from '../adaptation-panel-config'
 ...
 * @returns JSX Element
 */
export const AdaptationStatistics = ({ managingSystem, adaptationGoals }: Props) => {
   ...
   const tooltipIconContent = (
      <div style={tooltipContent}>
         ...
         <div>
            <span style={{ fontSize: '0.55rem' }}>{'\u2573'}</span> - goal is not fulfilled
         </div>
      </div>
   )

   const getIndicatorLabel = (label: string) => (
      <div className="system-indicator" onClick={() => setIsOpen((curr) => !curr)}>
         {label.toUpperCase()}
      </div>
   )
   ...
   const parseGoalQuality = (percentageVal: string, key: string, value: any) => {
      const qualityGoal = parseInt(key.split('Id')[1])
      const goal = adaptationGoals.filter((goal) => goal.id === qualityGoal)[0]

      if (goal) {
         const isGoalFulfilled = goal.isAboveThreshold ? goal.threshold <= value : goal.threshold >= value
         ...
         return (
            <div style={goalWrapper}>
               <img style={iconStyle} src={icon} data-tooltip-id={tooltipIconId} alt="Goal icon" />
               <span style={textStyle} ata-tooltip-id={tooltipQualityId}>
                  {['(', percentageVal, ')'].join('')}
               </span>
               <InfoTooltip id={tooltipIconId} content={tooltipIconContent} header={tooltipIconHeader} />
               <InfoTooltip id={tooltipQualityId} header={tooltipPercentHeader} />
            </div>
         )
      } else return null
   }
   ...
   const generateDetailsFields = () => {
      ...
      return ADAPTATION_STATISTICS_FIELDS.map((field) => {
         ...
         const { parsedVal, parsedLabel, type } = parseStatisticsValue(key, value, label)
