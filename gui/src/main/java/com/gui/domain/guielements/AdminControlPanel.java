					...
					.forEach(agentNode -> {
						...
						if (agentNode.isManualEventEnabled()) {
							if (!agentNode.isDuringEvent()) {
								...
								agentNode.setEvent(
										new PowerShortageEvent(POWER_SHORTAGE, getTimeForEventOccurrence(), maxPower));
								agentNode.setDuringEvent(true);
							} else {
								agentNode.setEvent(new PowerShortageFinishEvent(POWER_SHORTAGE_FINISH,
										getTimeForEventOccurrence()));
