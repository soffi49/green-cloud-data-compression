...
public class CloudNetworkAgentNode extends AgentNode {
  ...
  public void updateGraphUI() {
    synchronized (node) {
      if (traffic.get() > 85) {
        ...
        node.setAttribute(
            "ui.class", concatenateStyles(List.of(LABEL_STYLE, style, CLOUD_NETWORK_MEDIUM_STYLE)));
      } else if (traffic.get() > 0) {
        node.setAttribute(
            "ui.class", concatenateStyles(List.of(LABEL_STYLE, style, CLOUD_NETWORK_LOW_STYLE)));
      } else {
        node.setAttribute(
            "ui.class",
            concatenateStyles(List.of(LABEL_STYLE, style, CLOUD_NETWORK_INACTIVE_STYLE)));
      }
    }
  }
  ...
  protected void initializeLabelsMap() {
    ...
    labelsMap.put(
        AgentNodeLabelEnum.SERVERS_NUMBER_LABEL,
        createListLabel(String.valueOf(serverAgents.size())));
    labelsMap.put(
        AgentNodeLabelEnum.CURRENT_MAXIMUM_CAPACITY_LABEL,
    ...
    labelsMap.put(
        AgentNodeLabelEnum.TOTAL_NUMBER_OF_CLIENTS_LABEL,
