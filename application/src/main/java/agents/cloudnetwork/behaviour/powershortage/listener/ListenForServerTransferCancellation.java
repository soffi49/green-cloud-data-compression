...
public class ListenForServerTransferCancellation extends CyclicBehaviour {
	 ...
	 */
	public ListenForServerTransferCancellation(final Agent myAgent, final AID chosenServerForTransfer,
		...
		this.messageTemplate = and(MatchPerformative(INFORM),
				and(MatchProtocol(CANCELLED_TRANSFER_PROTOCOL), MatchSender(affectedServer)));
	...
	public void action() {
		...
		if (Objects.nonNull(inform)) {
			try {
				...
				logger.info("[{}] Sending the request for transfer cancellation to {}", myAgent.getName(),
						chosenServerForTransfer.getLocalName());
				displayMessageArrow(myCloudNetworkAgent, chosenServerForTransfer);
				myCloudNetworkAgent.send(prepareJobPowerShortageInformation(powerShortageJob, chosenServerForTransfer,
