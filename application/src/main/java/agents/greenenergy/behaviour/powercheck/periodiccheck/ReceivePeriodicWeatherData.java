...
public class ReceivePeriodicWeatherData extends CyclicBehaviour {
    ...
    public ReceivePeriodicWeatherData(GreenEnergyAgent myGreenAgent, Behaviour parentBehaviour) {
        ...
        this.messageTemplate = and(and(MatchProtocol(PERIODIC_WEATHER_CHECK_PROTOCOL), MatchSender(myGreenAgent.getMonitoringAgent())),
                                   and(or(MatchPerformative(INFORM), MatchPerformative(REFUSE)), MatchConversationId(PERIODIC_WEATHER_CHECK_PROTOCOL)));
    ...
    public void action() {
        ...
        if (nonNull(message)) {
            try {
                ...
                logger.info("[{}] I didn't understand the response with the weather data", myGreenEnergyAgent.getName());
            }
    ...
    private void handleInform(final MonitoringData data) {
        ...
        if (availablePower < myGreenEnergyAgent.manage().getCurrentPowerInUseForGreenSource()) {
            ...
            myAgent.addBehaviour(new AnnounceWeatherPowerShortage(myGreenEnergyAgent, null, getCurrentTime(), availablePower));
        } else {
