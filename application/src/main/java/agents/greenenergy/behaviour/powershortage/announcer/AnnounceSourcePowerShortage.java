...
public class AnnounceSourcePowerShortage extends OneShotBehaviour {
	 ...
	 */
	public AnnounceSourcePowerShortage(GreenEnergyAgent myAgent, OffsetDateTime shortageStartTime,
			int maxAvailablePower) {
		super(myAgent);
	...
	@Override
	public void action() {
		logger.info("[{}] !!!!! Power shortage was detected! Power shortage will happen at: {}", myGreenAgent.getName(),
				shortageStartTime);
		...
		if (affectedJobs.isEmpty()) {
			...
			myGreenAgent.addBehaviour(
					SchedulePowerShortage.createFor(shortageStartTime, maxAvailablePower, myGreenAgent));
		} else {
			...
			final List<PowerJob> jobsToTransfer = affectedJobs.stream().filter(job -> !jobsToKeep.contains(job))
					.toList();
			...
			myGreenAgent.addBehaviour(
					SchedulePowerShortage.createFor(preparePowerShortageTransfer(jobsToTransfer), maxAvailablePower,
							myGreenAgent));
		}
	}
