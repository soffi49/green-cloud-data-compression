...
public class RequestPowerJobTransfer extends AchieveREInitiator {
	...
	protected void handleInform(ACLMessage agree) {
		if (myGreenAgent.getPowerJobs().containsKey(jobToTransfer)) {
			...
			final boolean willJobFinishBeforeTransfer =
					jobToTransfer.getEndTime().isBefore(powerShortageStart) || jobToTransfer.getEndTime()
							.isEqual(powerShortageStart);

			if (willJobFinishBeforeTransfer) {
				logger.info("[{}] Job with id {} will finish before the transfer. Sending cancel transfer information ",
						guid, jobId);
				displayMessageArrow(myGreenAgent, myGreenAgent.getOwnerServer());
				myGreenAgent.send(
						prepareJobPowerShortageInformation(createPowerShortageJob(), myGreenAgent.getOwnerServer(),
								CANCELLED_TRANSFER_PROTOCOL));
			} else {
	...
	protected void handleFailure(ACLMessage failure) {
		if (myGreenAgent.getPowerJobs().containsKey(jobToTransfer)) {
			...
			final Behaviour requestBehaviour = new RequestPowerJobTransfer(myGreenAgent, transferRequest, jobToTransfer,
					powerShortageStart);
			myGreenAgent.addBehaviour(new RetryPowerJobTransferRequest(myGreenAgent, POWER_JOB_RETRY_TIMEOUT,
					JobMapper.mapToJobInstanceId(jobToTransfer), requestBehaviour));
			myGreenAgent.removeBehaviour(this);

		} else {
			logger.info("[{}] The job with id {} has finished before putting it on hold.", guid,
