...
public class ListenForServerPowerInformation extends CyclicBehaviour {
	...
	private void handleServerJobTransferFailure(final ACLMessage inform) {
		try {
			...
			if (Objects.nonNull(
					myGreenEnergyAgent.manage().getJobByIdAndStartDate(powerShortageJob.getJobInstanceId()))) {
				...
				final PowerJob jobToPutOnHold = myGreenEnergyAgent.manage()
				...
				logger.info("[{}] Job {} to put on hold was not found", myGreenEnergyAgent.getLocalName(),
	...
	private void handleCancelledPowerTransfer(final ACLMessage inform) {
		try {
			final PowerShortageJob powerShortageJob = getMapper().readValue(inform.getContent(),
					PowerShortageJob.class);
			final PowerJob jobToCancel = myGreenEnergyAgent.manage()
			...
			if (Objects.nonNull(jobToCancel)) {
				logger.info("[{}] Cancelling the job with id {}", myGreenEnergyAgent.getLocalName(),
				...
				logger.info("[{}] Job {} to cancel was not found", myGreenEnergyAgent.getLocalName(),
						powerShortageJob.getJobInstanceId().getJobId());
			}
		} catch (Exception e) {
			e.printStackTrace();
		...
		try {
			...
			if (Objects.nonNull(
				...
				logger.info(
						...
						myGreenEnergyAgent.getLocalName(), powerShortageJob.getJobInstanceId().getJobId());
				final PowerJob jobToDivide = myGreenEnergyAgent.manage()
						.getJobByIdAndStartDate(powerShortageJob.getJobInstanceId());
				myGreenEnergyAgent.manage()
						.divideJobForPowerShortage(jobToDivide, powerShortageJob.getPowerShortageStart());
			} else {
	...
	private void handleFinishedPowerShortage(final ACLMessage inform) {
		try {
			final JobInstanceIdentifier jobInstanceId = getMapper().readValue(inform.getContent(),
			...
			if (Objects.nonNull(powerJob)) {
				logger.info("[{}] Power shortage in server finished. Changing the status of the power job {}",
						myGreenEnergyAgent.getLocalName(), jobInstanceId.getJobId());
				...
				logger.info("[{}] Job {} to supply with green power was not found", myGreenEnergyAgent.getLocalName(),
