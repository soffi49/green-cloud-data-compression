...
public class StartJobExecution extends WakerBehaviour {
    ...
    public static StartJobExecution createFor(final ServerAgent serverAgent,
                                              ...
                                              final boolean informCNAStart,
                                              final boolean informCNAFinish) {
        final OffsetDateTime startDate = getCurrentTime().isAfter(job.getStartTime()) ? getCurrentTime() : job.getStartTime();
        return new StartJobExecution(serverAgent, Date.from(startDate.toInstant()), job, informCNAStart, informCNAFinish);
    }
    ...
    protected void onWake() {
        if (!myServerAgent.getGreenSourceForJobMap().containsKey(jobToExecute.getJobId())) {
            ...
            logger.info("[{}] Job execution couldn't start: job {} is not present", myServerAgent.getLocalName(), jobToExecute.getJobId());
        } else {
            if (informCNAStart) {
            ...
            final List<AID> receivers = informCNAStart ? List.of(myServerAgent.getGreenSourceForJobMap().get(jobToExecute.getJobId()), myServerAgent.getOwnerCloudNetworkAgent()) :
