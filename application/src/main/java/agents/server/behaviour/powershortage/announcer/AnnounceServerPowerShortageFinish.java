        ...
        if (jobsOnHold.isEmpty()) {
            ...
            jobsOnHold.forEach(job -> {
                if (myServerAgent.getServerJobs().containsKey(job) && myServerAgent.getGreenSourceForJobMap().containsKey(job.getJobId())) {
                    if (myServerAgent.manage().getAvailableCapacity(job.getStartTime(), job.getEndTime(), JobMapper.mapToJobInstanceId(job)) < job.getPower()) {
                        logger.info("[{}] There is not enough available power to bring job to in progress! Checking for back up power", myServerAgent.getLocalName());
                        if (myServerAgent.manage().getBackUpAvailableCapacity(job.getStartTime(), job.getEndTime(), JobMapper.mapToJobInstanceId(job)) < job.getPower()) {
                            logger.info("[{}] There is not enough available power to supply job with back up power! Leaving job on hold", myServerAgent.getLocalName());
                        } else {
