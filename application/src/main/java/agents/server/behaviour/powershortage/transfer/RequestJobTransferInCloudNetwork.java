	...
	protected void handleInform(ACLMessage agree) {
		if (Objects.nonNull(myServerAgent.manage().getJobByIdAndStartDate(jobToTransfer.getJobInstanceId()))) {
			...
			final boolean willJobFinishBeforeTransfer =
					jobOnBackUp.getEndTime().isBefore(jobToTransfer.getPowerShortageStart()) ||
			...
			if (willJobFinishBeforeTransfer) {
				...
				myServerAgent.send(
						prepareJobPowerShortageInformation(jobToTransfer, myServerAgent.getOwnerCloudNetworkAgent(),
								CANCELLED_TRANSFER_PROTOCOL));
				if (!isServerTransfer) {
					myServerAgent.send(prepareReply(greenSourceRequest.createReply(), jobToTransfer.getJobInstanceId(),
							ACLMessage.FAILURE));
				}
			} else {
