...
public class AnnounceSourceJobTransfer extends ContractNetInitiator {
    ...
    protected void handleAllResponses(Vector responses, Vector acceptances) {
        ...
        if (responses.isEmpty()) {
            ...
            myServerAgent.send(prepareReply(greenSourceRequest.createReply(), jobTransfer.getJobInstanceId(), ACLMessage.FAILURE));
        } else if (proposals.isEmpty()) {
            logger.info("[{}] No green sources are available for the power transfer of job {}. Passing the information to the cloud network",
            ...
            if (!validProposals.isEmpty()) {
                ...
                myServerAgent.addBehaviour(new ListenForSourceJobTransferConfirmation(myServerAgent, jobTransfer.getJobInstanceId(), greenSourceRequest));
                myAgent.send(ReplyMessageFactory.prepareAcceptReplyWithProtocol(chosenGreenSourceOffer.createReply(), jobTransfer.getJobInstanceId(), POWER_SHORTAGE_JOB_CONFIRMATION_PROTOCOL));
