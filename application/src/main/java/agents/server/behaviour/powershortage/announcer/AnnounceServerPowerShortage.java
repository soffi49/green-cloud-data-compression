...
public class AnnounceServerPowerShortage extends OneShotBehaviour {
	 ...
	 */
	public AnnounceServerPowerShortage(ServerAgent myAgent, OffsetDateTime shortageStartTime,
			int recalculatedAvailablePower) {
		super(myAgent);
	...
	@Override
	public void action() {
		logger.info("[{}] !!!!! Power shortage was detected for server! Power shortage will happen at: {}",
				myServerAgent.getName(), shortageStartTime);
		...
		if (affectedJobs.isEmpty()) {
			...
			myServerAgent.addBehaviour(
					HandleServerPowerShortage.createFor(affectedJobs, shortageStartTime, myServerAgent,
			...
			jobsToDivide.forEach(job -> {
				logger.info("[{}] Requesting job {} transfer in cloud network", myServerAgent.getName(),
						job.getJobId());
				...
				final ACLMessage transferMessage = preparePowerShortageTransferRequest(
						JobMapper.mapToPowerShortageJob(job, shortageStartTime),
						myServerAgent.getOwnerCloudNetworkAgent());
				displayMessageArrow(myServerAgent, myServerAgent.getOwnerCloudNetworkAgent());
				myServerAgent.addBehaviour(new RequestJobTransferInCloudNetwork(myServerAgent, transferMessage, null,
						JobMapper.mapToPowerShortageJob(jobToTransfer, shortageStartTime), true));
				informGreenSourceAboutPowerShortage(job, shortageStartTime);
			});
			myServerAgent.addBehaviour(
					HandleServerPowerShortage.createFor(jobsToDivide, shortageStartTime, myServerAgent,
							recalculatedAvailablePower));
		}
	}
	...
	private void informGreenSourceAboutPowerShortage(final Job job, final OffsetDateTime shortageTime) {
		...
		myServerAgent.send(
				prepareJobPowerShortageInformation(JobMapper.mapToPowerShortageJob(job, shortageTime), greenSource,
	...
	private List<Job> getAffectedPowerJobs() {
		return myServerAgent.getServerJobs().keySet().stream()
				.filter(job -> shortageStartTime.isBefore(job.getEndTime()) && !myServerAgent.getServerJobs().get(job)
