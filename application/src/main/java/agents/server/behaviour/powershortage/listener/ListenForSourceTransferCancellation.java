...
public class ListenForSourceTransferCancellation extends CyclicBehaviour {
	 ...
	 */
	public ListenForSourceTransferCancellation(final Agent myAgent, final AID chosenGreenSourceForTransfer,
			final AID affectedGreenSource) {
		super(myAgent);
		this.myServerAgent = (ServerAgent) myAgent;
		this.chosenGreenSourceForTransfer = chosenGreenSourceForTransfer;
		this.messageTemplate = and(MatchPerformative(INFORM),
				and(MatchProtocol(CANCELLED_TRANSFER_PROTOCOL), MatchSender(affectedGreenSource)));
		...
		if (Objects.nonNull(inform)) {
			try {
				...
				logger.info("[{}] Sending the information about job {} transfer cancellation to {}",
						myAgent.getName(), powerShortageJob.getJobInstanceId().getJobId(),
						chosenGreenSourceForTransfer.getLocalName());
				displayMessageArrow(myServerAgent, chosenGreenSourceForTransfer);
				myServerAgent.send(prepareJobPowerShortageInformation(powerShortageJob, chosenGreenSourceForTransfer,
